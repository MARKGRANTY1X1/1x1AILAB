...
const UsersPage: React.FC = () => {
  ...
  const handleAddUser = (newUser: Omit<User, 'id' | 'role' | 'status' | 'lastSeen' | 'settings'> & { walletAddress: string }): boolean => {
                ...
                <tr key={user.id}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <span
                        className={`h-2.5 w-2.5 rounded-full mr-3 flex-shrink-0 ${
                          user.status === 'offline' ? 'bg-red-500' : 'bg-green-500'
                        }`}
                        title={user.status === 'offline' ? 'Offline' : (user.status === 'mining' ? 'Mining' : 'Online')}
                      ></span>
                      <div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-light-text-secondary dark:text-dark-text-secondary">
                    {user.settings.walletAddress ? <span className="text-green-500">Address Set</span> : <span className="text-yellow-500">Not Set</span>}
                  </td>
